...
**GetFinancialsNewsFromBoursoInput**
...
**GetFinancialsNewsFromBoursoOutput**: `items: list[NewsItemOutput]`, `count: int`
...
Useful for ETF analysis when combined with `get_financials_news_from_bourso` and composition holdings.
...
### Requirement: ToolRegistry exposes Bourso market tools
`ToolRegistry.all_tools()` MUST include: `get_financials_news_from_bourso`, `get_data_from_url`, `search_boursorama`, `get_etf_composition`, `update_index_boursorama`.

#### Scenario: Registry construction
- **WHEN** `ToolRegistry` is built at app bootstrap
- **THEN** all five Bourso market tools are present in `all_tools()`
...
| Portfolio Manager | no URL/search/ETF tools (calendar fetch wired separately) |

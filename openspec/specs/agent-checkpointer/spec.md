...
#### Scenario: Docker test suite uses Postgres
- **WHEN** `just test` runs `api/tests` and `agentic/agentic_tests`
- **THEN** a dedicated `pgvector/pg16` service provides `nam_test`
- **AND** bootstrap and lifespan tests use the same Postgres checkpointer

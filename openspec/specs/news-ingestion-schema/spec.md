...
| Value | Use |
|-------|-----|
| `CALENDAR_GENERAL` | Legacy calendar category (no longer cron-ingested) |
| `CALENDAR_LISTED_COMPANIES` | Legacy calendar category |
| `CALENDAR_MACRO` | Legacy calendar category |
| `CALENDAR_DIVIDENDS` | Legacy calendar category |
...
#### Scenario: Category covers session ingest feeds
- **WHEN** a session news ingest completes
- **THEN** persisted rows use `MARKETS` or `FINANCE` matching the source list page

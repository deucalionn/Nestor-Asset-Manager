...
`GetPortfolioPositionsTool` MUST depend on a `MarketPriceProvider` protocol. A stub returning `None` MUST ship for production v1; tests MUST use a fake provider to validate PnL math.

#### Scenario: Stub provider in production
- **WHEN** the default stub provider is used
- **THEN** `gain_loss_pct` is `null` for all positions

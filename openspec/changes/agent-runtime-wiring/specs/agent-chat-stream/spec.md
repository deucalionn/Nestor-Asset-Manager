...
`nam-agentic` MUST expose `POST /chat/stream` that accepts a JSON body and returns a **streaming HTTP response** (chunked body with newline-delimited JSON events or SSE).
...
`/chat/stream` MUST call `AgentRunner.stream()` with `NamRuntimeContext(phase=MarketPhase.CHAT, thread_id=..., user_id=...)`.

#### Scenario: Runtime context for chat
- **WHEN** `/chat/stream` processes a message
- **THEN** `AgentRunner.stream()` receives `phase=CHAT`
